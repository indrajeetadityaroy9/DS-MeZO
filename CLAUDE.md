...
python scripts/prepare_pissa.py --model <model_path> --output <output_dir> [--targets q_proj v_proj]

# Training (requires GPU, vLLM)
bash scripts/launch.sh <config.yaml> <prompts.jsonl>

# Evaluation scripts (each self-contained, requires GPU + PiSSA-prepared model at /dev/shm/pissa_prep)
python eval/rl_bench_eval.py    # Llama-3.1-8B MBPP pass@1 (1000 steps) — primary RL proof-of-concept
python eval/sft_eval.py         # GSM8K perplexity — SFT baseline
python eval/ablations.py        # Component ablation experiments (8 controlled experiments)

# Tests (require CUDA GPU)
python tests/test_evaluation.py  # Mathematical property verification (orthogonality, convergence, etc.)
```

## Architecture

**Two-process design**: The controller (FP32 master weights, optimizer state) and vLLM engine (BF16 inference) communicate via adapter serialization to `/dev/shm/ds_mezo/`.
...
Controller (controller.py)          Backend (backend.py)
  ...
  RLOO advantage computation          Activation hook capture
       |                                    |
       +--- serialize adapters via /dev/shm ---+
```

### Core modules (`ds_mezo/`)

- **`controller.py`** — Main training loop. Two modes: `rl` (AGZO subspace perturbation + ZO-Muon + RLOO) and `sft` (vanilla MeZO + SGD). Config via `DSMeZOConfig` dataclass. Layer state via `LayerState` dataclass. Algorithm-only — no vLLM imports.
...
- **Adapter staging**: Adapters serialize to `/dev/shm/ds_mezo/` (tmpfs) for zero-copy vLLM loading. Two adapter slots: `adapter_pos` (θ+ε) and `adapter_neg` (θ-ε) for SPSA.
...
- `VLLM_ALLOW_INSECURE_SERIALIZATION=1` required (set in launch.sh and eval scripts)
- Config files use YAML; only `model_path` and `adapter_path` are required, all else defaults from `DSMeZOConfig`
