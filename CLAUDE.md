...
A minimal smoke-test config exists at `configs/smoke.yaml` (10 steps, requires PiSSA-prepared model at `/dev/shm/pissa_prep/`).
...
- **`kernels.py`** — Two Triton kernels: `zo_muon_update` (fused gradient + momentum + 5-iteration Newton-Schulz orthogonalization + param update) and `fused_perturb_dual` (compute θ+ and θ- in one pass). Dispatches tall vs wide kernel based on matrix shape.
...
### `DSMeZOConfig` fields (config YAML surface)
...
Epsilon is derived from adapter rank. DD clipping uses adaptive EMA variance tracking. See `DSMeZOConfig` docstring.
...
- `launch.sh` also locks GPU clocks (`nvidia-smi -lgc`) and sets thread counts for reproducibility
